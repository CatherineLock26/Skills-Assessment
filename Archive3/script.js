...
async function loginUser() {
  ...
  if (!username || !password) return userLoginMessage.textContent = "Enter your username and password.";

  const { error } = await sb.auth.signInWithPassword({ email: usernameToEmail(username), password });
  if (error) return userLoginMessage.textContent = "Incorrect username or password.";
